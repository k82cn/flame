...
export RELEASE_NOTES_FILE=/private/tmp/flame-${RELEASE_TAG}-notes.md
```
...
Confirm credentials before changing versions:
...
uv build --out-dir /private/tmp/flamepy-${PYTHON_VERSION}-dist
cd -
```
...
cd sdk/python
uv build --out-dir /private/tmp/flamepy-${PYTHON_VERSION}-dist
uv publish /private/tmp/flamepy-${PYTHON_VERSION}-dist/*
cd -
```
...
Build both platforms into each manifest:
...
Inspect local manifests before pushing:
...
Verify Docker Hub exposes both architectures:
...
the affected platform before rebuilding:
...
```shell
IMAGE_REGISTRY=xflops IMAGE_TAG="${DOCKER_TAG}" ci/k8s/e2e.sh
```
