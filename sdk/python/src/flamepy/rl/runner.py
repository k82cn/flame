...
class RunnerService:
    ...
    def __init__(self, app: str, execution_object: Any, kind: Optional[RunnerServiceKind] = None):
        ...
        runner_context = RunnerContext(
            execution_object=execution_object, kind=kind)
        ...
        self._session = create_session(
            application=app, common_data=common_data_bytes, session_id=temp_session_id)
...
class Runner:
    ...
    def __enter__(self) -> "Runner":
        ...
        try:
            ...
            logger.debug(
                f"Registered application '{self._name}' with working directory: {working_directory}")
        except Exception as e:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb) -> None:
        ...
        if self._app_registered:
            try:
                ...
                logger.error(
                    f"Error unregistering application: {e}", exc_info=True)
        ...
        if self._package_path and os.path.exists(self._package_path):
            try:
                ...
                logger.error(
                    f"Error removing local package: {e}", exc_info=True)
    ...
    def service(self, execution_object: Any, kind: Optional[RunnerServiceKind] = None) -> RunnerService:
        ...
        logger.debug(
            f"Created service for execution object in Runner '{self._name}'")
    ...
    def _upload_package(self) -> str:
        ...
        dest_path = os.path.join(
            storage_dir, os.path.basename(self._package_path))

        # Check if package already exists
        if os.path.exists(dest_path):
            logger.debug(
                f"Package already exists at {dest_path}, skipping upload")
        else:
            try:
