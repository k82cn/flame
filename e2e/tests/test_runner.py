...
def test_runner_with_function(check_package_config, check_flmrun_app):
    ...
    with flamepy.Runner("test-runner-func") as rr:
        ...
        assert isinstance(result, flamepy.ObjectFuture), f"Expected ObjectFuture, got {type(result)}"
...
def test_runner_with_class(check_package_config, check_flmrun_app):
    ...
    with flamepy.Runner("test-runner-class") as rr:
        ...
        # Call methods - use ref() to ensure completion without fetching results
        cnt_s.increment().ref()
        cnt_s.add(3).ref()
...
def test_objectfuture_ref_method(check_package_config, check_flmrun_app):
    ...
    with flamepy.Runner("test-objectfuture-ref") as rr:
        ...
        assert isinstance(obj_ref, flamepy.ObjectRef), f"Expected ObjectRef, got {type(obj_ref)}"
...
def test_runner_service_close(check_package_config, check_flmrun_app):
    """Test Case 10: Test that RunnerService.close() works."""
...
def test_flame_package_dataclass():
    """Test Case 11: Test FlamePackage dataclass."""
    # Test with defaults
...
def test_runner_error_no_package_config():
    """Test Case 12: Test Runner fails gracefully without package config."""
